...
GENO_NONSYN_TSV="$OUTDIR/genotypes_for_load_nonsyn.tsv"
LOAD_TABLE_NONSYN="$OUTDIR/nonsyn_per_sample.tsv"

echo "File checks:"
wc -l "$SAMPLES"
wc -l "$GENO_NONSYN_TSV"
ls -lh "$GENO_NONSYN_TSV"
echo
...
nonsyn_geno_file = os.path.join(outdir, "genotypes_for_load_nonsyn.tsv")
nonsyn_load_out = os.path.join(outdir, "nonsyn_per_sample.tsv")
...
sys.stderr.write(f"Reading genotypes from: {nonsyn_geno_file}\n")
line_count = 0
with open(nonsyn_geno_file) as f:
        for line in f:
                line = line.rstrip("\n")
                if not line:
                        continue
...
with open(nonsyn_load_out, "w") as out:
	out.write("Sample\tNonSynonymousCounts\n")
	for s, l in zip(samples, loads):
		out.write(f"{s}\t{l}\n")
sys.stderr.write(f"Output written to: {nonsyn_load_out}\n")
EOF
